...
public class StringSet {
    public static TreeSet<String> findUniqueWords(){
        String sentence = "Carroll began writing the manuscript of the story the next day, although that earliest version is lost. " +
        ...
        sentence = sentence
                .replace(".", "") // Replace all . with nothing
                .replace(",", "") // Replace all , with nothing
                .replace("!", "") // Replace all ! with nothing
                .replace("?", "") // Replace all ? with nothing
                .replace("\"", "") // Replace all " with nothing via escaping \"
                .replaceAll("(\s*[0-9]+)+", ""); // Replace all digits occuring one or many times

        String [] words = sentence.split(" "); // Split sentences with space character

        List<String> loweredWords = new ArrayList<>();

        for(String word : words){
            loweredWords.add(word.toLowerCase()); // Lower case all words and add to loweredWords array list
        }

        return new TreeSet<>(loweredWords);// sorting
    }
}
